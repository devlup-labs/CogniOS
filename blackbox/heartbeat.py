...
def mark_graceful_shutdown(conn: sqlite3.Connection) -> None:
    # Mark the graceful shutdown flag in the heartbeat table
    conn.execute(
...
def check_crash_on_startup(conn: sqlite3.Connection) -> tuple[bool, float]:
   ...
   # check if the last run ended gracefully or if there was a crash based on the heartbeat timestamp and graceful shutdown flag.
    row = conn.execute(
