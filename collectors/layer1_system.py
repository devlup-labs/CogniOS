...
def collect_layer1_metrics():
	...
	sockets = {"tcp_sockets_inuse": 0, "udp_sockets_inuse": 0}
	try:
		with open("/proc/net/sockstat", "r") as f:
			for line in f:
				#  "TCP: inuse 45 orphan 0 tw 12 alloc 89 mem 12"
				if line.startswith("TCP:"):
					sockets["tcp_sockets_inuse"] = int(line.split()[2])
				#  "UDP: inuse 18 mem 8"
				elif line.startswith("UDP:"):
					sockets["udp_sockets_inuse"] = int(line.split()[2])
	...
	try:
		...
		pass
	psi_cpu_some, psi_mem_some, psi_io_some = 0, 0, 0
	try:
		psi_cpu_some=read_psi_metric("cpu")
		psi_mem_some=read_psi_metric("mem")
		psi_io_some=read_psi_metric("io")
	except Exception:
		pass
	network_symmetry=net_bytes_sent/(net_bytes_sent+net_bytes_received)
	return {
		"timestamp": timestamp,
