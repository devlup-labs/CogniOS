...
def render():
    ...
    with col_target:
        ...
        st.html(f"""<div style="background:#0d121c; border-radius:14px; padding:24px; min-height: 380px; box-shadow:0 6px 24px rgba(0,0,0,0.3);"><div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;"><div><h3 style="margin:0; font-size:20px; font-weight:700; color:#ffffff;">Target Workload</h3><p style="margin:2px 0 0 0; font-size:12px; color:#64748b;">Real-time resource allocation</p></div><div style="background: rgba(0, 245, 196, 0.12); color:#00f5c4; border-radius:6px; padding:6px 14px; font-size:12px; font-weight:700; font-family:\'JetBrains Mono\';">● ACTIVE STATE: {workload_data['workload']} ({workload_data['confidence']}% CONF)</div></div><div style="margin-top:24px;"><div style="font-size:12px; font-weight:700; color:#64748b; font-family:\'JetBrains Mono\'; text-transform:uppercase; margin-bottom:16px;">Processor Affinity Matrix</div><div style="margin-bottom:20px;"><div style="display:flex; justify-content:space-between; font-size:13px; color:#cbd5e1; margin-bottom:8px;"><span><strong style="color:#ffffff;">Performance Pool (P-Cores)</strong></span><span style="color:#00f5c4; font-family:\'JetBrains Mono\'; font-weight:600;">{affinity_data['p_active']}/{affinity_data['p_active']} Active</span></div><div style="display:grid; grid-template-columns: repeat(4, 1fr); gap:10px;">{p_boxes}</div></div><div><div style="display:flex; justify-content:space-between; font-size:13px; color:#cbd5e1; margin-bottom:8px;"><span><strong style="color:#ffffff;">Efficiency Pool (E-Cores)</strong></span><span style="color:#64748b; font-family:\'JetBrains Mono\'; font-weight:600;">{affinity_data['e_active']}/8 Active</span></div><div style="display:grid; grid-template-columns: repeat(4, 1fr); gap:10px;">{e_boxes}</div></div></div>{llm_html}</div>""")
