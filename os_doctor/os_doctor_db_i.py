...
def write_to_os_doctor_train_i(ml_features_df, timestamp, os_doctor_db_path):
...
def execute_os_doctor_db():
    ...
    try:
        print("Starting the appending process for os_doctor_train_i (idle). Press Ctrl+C to stop")
        while True:
            try:
                ...
                if ml_features_df is not None:
                    ...
                    if timestamp == last_written_timestamp:
                        print("[idle] No new telemetry since the last row. Skipping.")
                    else:
                        write_to_os_doctor_train_i(ml_features_df, timestamp, OS_DOCTOR_DB_PATH)
                        last_written_timestamp = timestamp
                        print("[idle] Successfully appended to os_doctor_train_i")
