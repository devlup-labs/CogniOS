...
def write_to_os_doctor_train_b(ml_features_df, timestamp, os_doctor_db_path):
...
def execute_os_doctor_db():
    ...
    try:
        print("Starting the appending process for os_doctor_train_b (browsing). Press Ctrl+C to stop")
        while True:
            try:
                ...
                if ml_features_df is not None:
                    ...
                    if timestamp == last_written_timestamp:
                        print("[browsing] No new telemetry since the last row. Skipping.")
                    else:
                        write_to_os_doctor_train_b(ml_features_df, timestamp, OS_DOCTOR_DB_PATH)
                        last_written_timestamp = timestamp
                        print("[browsing] Successfully appended to os_doctor_train_b")
