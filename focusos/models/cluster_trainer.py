...
def save_models(model: KMeans, scaler: StandardScaler, save_dir: str = MODELS_DIR) :
    #"""
...
def load_from_csv(csv_path: str) -> pd.DataFrame:
    """
    Loads pre-generated synthetic feature vectors from CSV.
    
    Used when real telemetry data is insufficient or unavailable.
    The CSV is generated by generate_dataset.py which produces
    realistic feature vectors for each workload class.
    
    ARGS:
        csv_path: path to focusos_training_data.csv
        
    RETURNS:
        DataFrame with feature columns only (no label column)
        Labels are stripped here because KMeans is unsupervised —
        it should discover clusters without seeing the labels.
        Labels come back in via generate_pseudo_labels() after
        human inspection of clusters.
    """
    ...
    print(f"[load_csv] Loaded {len(df)} rows from {csv_path}")
    
    # drop the label column — KMeans should not see it
    # we want it to discover clusters on its own
    if "workload_label" in df.columns:
        df = df.drop(columns=["workload_label"])
        print(f"[load_csv] Dropped workload_label column "
              f"(KMeans is unsupervised)")
    
    # keep only the feature columns in correct order
    available = [c for c in FEATURE_COLUMNS if c in df.columns]
    missing = [c for c in FEATURE_COLUMNS if c not in df.columns]
    
    if missing:
        print(f"[load_csv] WARNING: Missing features: {missing}")
    
    df = df[available]
    print(f"[load_csv] Final shape: {df.shape}")
    return df
...
def run_training_pipeline(
    ...
    if csv_path is not None:
        # use synthetic dataset from generate_dataset.py
        print(f"[pipeline] Using CSV dataset: {csv_path}")
        df_features = load_from_csv(csv_path)
    else:
        # use real telemetry from DB
        print(f"[pipeline] Using DB telemetry: {db_path}")
        df_features = collect_feature_vectors(
            db_path=db_path, 
            n_samples=n_samples
        )
...
if __name__ == "__main__":
     ...
     # path to synthetic dataset generated by generate_dataset.py
    CSV_PATH = os.path.join(
        os.path.dirname(os.path.abspath(__file__)),
        "focusos_training_data.csv"
    )
    
    # check if CSV exists
    if not os.path.exists(CSV_PATH):
        print("CSV not found. Run this first:")
        print("  python -m focusos.models.generate_dataset")
        exit(1)
    """
    TYPICAL WORKFLOW:
    ...
    run_training_pipeline(
        ...
        run_elbow=True,          # set True on first run to verify k=5
